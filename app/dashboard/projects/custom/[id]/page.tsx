...
export default function CustomProjectPage() {
  ...
  const [projectInfo, setProjectInfo] = useState<CustomProject | null>(null);
  ...
  // プロジェクト情報を取得
  useEffect(() => {
    const customProjects = localStorage.getItem("customProjects");
    if (customProjects) {
      const projects: CustomProject[] = JSON.parse(customProjects);
  ...
  // プロジェクトデータを取得（projectInfoが設定された後）
  useEffect(() => {
    ...
    if (stored) {
      ...
      localStorage.setItem(`project_${projectId}`, JSON.stringify(initialData));
    }
  ...
  const addTask = () => {
    if (!taskText || !taskStartDate || !taskDeadline || !project) return;
  ...
  const editTask = (task: ProjectTask) => {
    ...
    setTaskHours(task.hours !== undefined ? String(task.hours) : "");
    setEditingTask(task);
    setShowTaskForm(true);
  };

  const deleteTask = (id: string) => {
    if (!confirm("タスクを削除しますか？") || !project) return;
    saveProject({
      ...project,
    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div className="mb-2">
            <span className="text-sm text-gray-600">合計工数: <strong>{project.tasks.reduce((s, t) => s + (t.hours || 0), 0)}</strong> 時間</span>
          </div>
      ...
      {/* 外部リンク */}
      <ExternalLinks storageKey={`custom-${projectId}`} />
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        ...
        {showTaskForm && (
          <div className="mb-4 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="mb-3">
              <label className="block text-sm text-gray-600 mb-1">工数（時間）</label>
              <input
                type="number"
                step="0.5"
                min="0"
