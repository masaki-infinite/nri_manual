...
const PROJECT_ID = "github-external";
const PROJECT_NAME = "GitHub外販";

export default function GitHubExternalProjectPage() {
  ...
  const [companyMemos, setCompanyMemos] = useState<{[key: string]: string}>({});
  ...
  useEffect(() => {
    ...
    // 会社別メモの読み込み
    const storedMemos = localStorage.getItem(`${PROJECT_ID}_company_memos`);
    if (storedMemos) {
      setCompanyMemos(JSON.parse(storedMemos));
    }
  }, []);
  ...
  const updateCompanyMemo = (companyName: string, memo: string) => {
    const updated = { ...companyMemos, [companyName]: memo };
    setCompanyMemos(updated);
    localStorage.setItem(`${PROJECT_ID}_company_memos`, JSON.stringify(updated));
  };
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 mb-6">
        ...
        <div className="space-y-3">
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            <button
              onClick={() => toggleAccordion("テレ・マーカーグループ株式会社")}
              className="w-full px-4 py-3 bg-gray-50 hover:bg-gray-100 transition-colors flex items-center justify-between"
            >
              <span className="font-semibold text-gray-800">テレ・マーカーグループ株式会社</span>
              <span className="text-gray-600">
                {openAccordions.has("テレ・マーカーグループ株式会社") ? "▼" : "▶"}
              </span>
            </button>
            {openAccordions.has("テレ・マーカーグループ株式会社") && (
              <div className="p-4 bg-white border-t border-gray-200">
                <textarea
                  value={companyMemos["テレ・マーカーグループ株式会社"] || ""}
                  onChange={(e) => updateCompanyMemo("テレ・マーカーグループ株式会社", e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  rows={5}
                  placeholder="テレ・マーカーグループ株式会社に関する情報を記入してください"
                />
              </div>
            )}
          </div>
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            <button
              onClick={() => toggleAccordion("株式会社エービッツ")}
              className="w-full px-4 py-3 bg-gray-50 hover:bg-gray-100 transition-colors flex items-center justify-between"
            >
              <span className="font-semibold text-gray-800">株式会社エービッツ</span>
              <span className="text-gray-600">
                {openAccordions.has("株式会社エービッツ") ? "▼" : "▶"}
              </span>
            </button>
            {openAccordions.has("株式会社エービッツ") && (
              <div className="p-4 bg-white border-t border-gray-200">
                <textarea
                  value={companyMemos["株式会社エービッツ"] || ""}
                  onChange={(e) => updateCompanyMemo("株式会社エービッツ", e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  rows={5}
                  placeholder="株式会社エービッツに関する情報を記入してください"
                />
              </div>
            )}
          </div>
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            <button
              onClick={() => toggleAccordion("株式会社スクウェア・エニックス")}
              className="w-full px-4 py-3 bg-gray-50 hover:bg-gray-100 transition-colors flex items-center justify-between"
            >
              <span className="font-semibold text-gray-800">株式会社スクウェア・エニックス</span>
              <span className="text-gray-600">
                {openAccordions.has("株式会社スクウェア・エニックス") ? "▼" : "▶"}
              </span>
            </button>
            {openAccordions.has("株式会社スクウェア・エニックス") && (
              <div className="p-4 bg-white border-t border-gray-200">
                <textarea
                  value={companyMemos["株式会社スクウェア・エニックス"] || ""}
                  onChange={(e) => updateCompanyMemo("株式会社スクウェア・エニックス", e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  rows={5}
                  placeholder="株式会社スクウェア・エニックスに関する情報を記入してください"
                />
              </div>
            )}
          </div>
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            <button
              onClick={() => toggleAccordion("株式会社プロトコーポレーション")}
              className="w-full px-4 py-3 bg-gray-50 hover:bg-gray-100 transition-colors flex items-center justify-between"
            >
              <span className="font-semibold text-gray-800">株式会社プロトコーポレーション</span>
              <span className="text-gray-600">
                {openAccordions.has("株式会社プロトコーポレーション") ? "▼" : "▶"}
              </span>
            </button>
            {openAccordions.has("株式会社プロトコーポレーション") && (
              <div className="p-4 bg-white border-t border-gray-200">
                <textarea
                  value={companyMemos["株式会社プロトコーポレーション"] || ""}
                  onChange={(e) => updateCompanyMemo("株式会社プロトコーポレーション", e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  rows={5}
                  placeholder="株式会社プロトコーポレーションに関する情報を記入してください"
                />
              </div>
            )}
          </div>
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            <button
              onClick={() => toggleAccordion("管清工業株式会社")}
              className="w-full px-4 py-3 bg-gray-50 hover:bg-gray-100 transition-colors flex items-center justify-between"
            >
              <span className="font-semibold text-gray-800">管清工業株式会社</span>
              <span className="text-gray-600">
                {openAccordions.has("管清工業株式会社") ? "▼" : "▶"}
              </span>
            </button>
            {openAccordions.has("管清工業株式会社") && (
              <div className="p-4 bg-white border-t border-gray-200">
                <textarea
                  value={companyMemos["管清工業株式会社"] || ""}
                  onChange={(e) => updateCompanyMemo("管清工業株式会社", e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  rows={5}
                  placeholder="管清工業株式会社に関する情報を記入してください"
                />
              </div>
            )}
          </div>
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            <button
              onClick={() => toggleAccordion("三菱電機ビルソリューションズ")}
              className="w-full px-4 py-3 bg-gray-50 hover:bg-gray-100 transition-colors flex items-center justify-between"
            >
              <span className="font-semibold text-gray-800">三菱電機ビルソリューションズ</span>
              <span className="text-gray-600">
                {openAccordions.has("三菱電機ビルソリューションズ") ? "▼" : "▶"}
              </span>
            </button>
            {openAccordions.has("三菱電機ビルソリューションズ") && (
              <div className="p-4 bg-white border-t border-gray-200">
                <textarea
                  value={companyMemos["三菱電機ビルソリューションズ"] || ""}
                  onChange={(e) => updateCompanyMemo("三菱電機ビルソリューションズ", e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  rows={5}
                  placeholder="三菱電機ビルソリューションズに関する情報を記入してください"
                />
                <div className="mt-3 text-sm text-gray-600">
                  <Link href="/dashboard/projects/github-external/mitsubishi-electric-building-solutions" className="text-indigo-600 hover:underline">詳しく見るページへ移動</Link>
                </div>
              </div>
            )}
          </div>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-2 text-sm text-gray-600">工数: {task.hours || 0} 時間</div>
